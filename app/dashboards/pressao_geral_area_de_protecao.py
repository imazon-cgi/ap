...
def register_pressao_area_protecao(flask_server):
    ...
    def atualizar(modalidade, uso, uf, reset, bar_click, map_click, selecionados):
        ...
        bar.update_layout(
            ...
            bargap=0.1,
            font=dict(size=10),
            title=dict(text="Top 10 Áreas de Proteção Ambiental <br> por Desmatamento", x=0.5, xanchor="center"),
        )
        ...
        pie_uso = px.pie(
            ...
            names="UF",
            color="MODALIDADE",
            title="Pressão Desmatamento por Estado e Modalidade",
        )
        ...
        pie_area = px.pie(
            ...
            names="NOME",
            color="UF",
            title="Pressão Desmatamento por Área de Proteção",
        )
        pie_area.update_traces(textinfo="none", hoverinfo="label+value+percent", marker=dict(colors=cores))
    ...
    def baixar_csv(n, sep, no_acc):
        ...
        return dcc.send_data_frame(out.to_csv, "pressao_area_protecao.csv", sep=sep, index=False)
